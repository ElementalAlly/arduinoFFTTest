...
void loop() {
  for (int i = 0; i < samples; i++) {
    vReal[i] = int16_t(signal1Amp * (sin(i * (twoPi * signal1Cycles) / samples)) + signal2Amp * (sin(i * (twoPi * signal2Cycles) / samples)) + signal3Amp * (sin(i * (twoPi * signal3Cycles) / samples)) + signal4Amp * (sin(i * (twoPi * signal4Cycles) / samples)) + signal5Amp * (sin(i * (twoPi * signal5Cycles) / samples)));
  }

  uint32_t startTime = micros();
  KickFFT<int16_t>::fft(samples, vReal, mags);
  uint32_t endTime = micros();
  uint32_t ETA = endTime - startTime;
  // uncomment when testing for accuracy
  for (int i = 0; i < samples / 2; i++) {
    Serial.println(mags[i]);
  }
  while(1);
